        ...
        function createD3(colony){
            ...
            groups = groups.map(function(name, n) {
                ...
                if(name == 1){
                    ...
                    color = "rgb(179, 171, 15)";
                }else if(name == 4){
                    color = "rgb(13, 158, 54)";
                }else if(name == 5){
                    color = "rgb(136, 202, 22)";
                }else if(name == 6){
                    color = "rgb(21, 19, 19)";
                }else if(name == 7){
                    color = "rgb(163, 10, 33)";
            ...
            function getStateColor(type){

            }

            var force = d3.layout.force()
                .size([width, height])
                .charge(-150 * colony.scale)
            ...
            function getNodeInfo(d){
                $("#readme-contents").slideUp(500,function(){
                    $("#taskName").html("任务名称:<span>"+d.englishName+"</span>");
                    $("#readme-contents").slideDown(500);
                });

                console.log(d.id);
                $.ajax({
                    type: "post",
                    url: "/sentosa/scheduler/find/condition",
                    data: {
                        taskId: d.id
                    },
                    success: function (result) {
                        ...
                        if(result && result.success){
                            ...
                            if(!dat){
                                showTip("此任务未添加调度.");
                                $("#englishName").html("调度名称:<span>无</span>");
                                $("#times").html("调度执行时间:<span>无</span>");
                                $("#status").html("状态:<span>无</span>");
                                $("#pinci").html("频次:<span>无</span>");
                                $("#readme-contents").attr("data-id", "");
                                $("#log").attr("href","javascript:void(0)");
                                $("#diaodu").attr("href","javascript:void(0)");
                                return;
                            }
                            $("#englishName").html("调度名称:<span>"+dat.englishName+"</span>");
                            $("#times").html("调度执行时间:<span>"+(new Date(dat.startExecTime)).Format("yyyy-MM-dd hh:mm:ss")+"</span>");
                            $("#status").html("状态:<span>"+getActive(dat.active)+"</span>");
                            $("#readme-contents").attr("data-id", dat.id);
                            $("#pinci").html("频次:<span>"+getCronType(dat.cronType)+"</span>");
                            $("#log").attr("href","/sentosa/page/task/taskHostoryList.html?taskId="+dat.taskId);
                            $("#diaodu").attr("href","/sentosa/page/task/taskList.html?id="+dat.id);
            ...
            $("#search").click(function(){
                var value = $("#tasknameValue").val();
                if($.trim(value) == ""){
                    return;
                }else{
                    node.style('opacity', function(o) {
                        o.active = o.englishName == value;
                        return o.active ? 1 : 0.2
                    })
                    ...
                    force.charge(function(o) {
                        ...
                        return (l.source.englishName == value || l.target.englishName == value ? 1 : 0) * colony.scale
                    }).start()
